...
@implementation TPFBlockStrongLayout
...
static NSIndexSet *_GetBlockStrongLayout(void *block) {
  ...
  for (size_t i = 0; i < elements; ++i) {
    TPFBlockStrongRelationDetector *detector = [TPFBlockStrongRelationDetector new];
  ...
  for (size_t i = 0; i < elements; ++i) {
    TPFBlockStrongRelationDetector *detector = (TPFBlockStrongRelationDetector *)(detectors[i]);
...
+ (NSArray *)TPFGetBlockStrongReferences:(void *)block {
  if (![self TPFObjectIsBlock:block]) {
    return nil;
  }
...
+ (BOOL)TPFObjectIsBlock:(void *)object {
